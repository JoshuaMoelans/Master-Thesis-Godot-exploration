...
func add_traversable_tiles(tiles: Array):
	for tile in tiles:
		...
		rect.size = 0.9*tilemap.tile_set.tile_size
...
func connect_traversable_tiles(tiles: Array):
	for tile in tiles:
		...
		# 8-neighbour grid approach
		#for x in range(3):
			#for y in range(3):
		# only horizontal/vertical neigbours
		for point in [Vector2i(0,1), Vector2i(0,-1),Vector2i(1,0),Vector2i(-1,0)]:
...
func update_navigation_map():
	...
	for obstacle in obstacles:
		...
		if obstacle is CharacterBody2D:  # avoid player/allies/enemies
...
func get_new_path(start: Vector2, end:Vector2) -> Array:
	...
	var path_world =[]
	for point in path_map:  # convert back to world coordinates
